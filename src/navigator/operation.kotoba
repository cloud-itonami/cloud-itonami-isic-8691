(ns navigator.operation
  "OperationActor -- one navigator operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (NavigatorOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Health Access Governor
  ...
  One graph run = one navigator operation (intake -> advise -> govern
  ...
  decision to a human operator (care-navigator/clinical lead). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/finalize-referral`/`:actuation/disclose-
  health-information` ALWAYS reach this node when the governor is
  clean -- see `navigator.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [navigator.navigatoradvisor :as navigatoradvisor]
...
(defn build
  ...
  opts:
    :advisor      -- a `navigator.navigatoradvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (navigatoradvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; NavigatorOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (navigatoradvisor/-advise advisor store request)]
            {:proposal p :audit [(navigatoradvisor/trace request p)]})))

      ;; Health Access Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
